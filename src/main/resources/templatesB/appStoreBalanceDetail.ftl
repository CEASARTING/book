                            ...
                            if (detail.length == 0) {
                                ...
                                for (var i = 0; i < detail.length; i++) {
                                    ...
                                    result += '<div class="user_13">'
                                            ...
                                            + '<span>现金余额：' + detail[i].detailBalance + '</span><span>' + actionType+detail[i].detailMoney + '</span></div>';
...
<script>
   $(function(){
	   $('.user_12_1').click(function(){
           ...
           //
           // $(".user_13").css("display","none");
           // $("."+className).css("display","block");
           //
           // $(this).addClass('checked').siblings().removeClass('checked');

           location.href="/store/balanceDetail/${storeId}/"+status;
...
<body class="body_1">
   <div class="header"> <img onclick=window.location.href="/store/balance/${storeId!}" src="/images/27.png"><span>明细</span> </div>
   <div class="user_12">
      <div class="user_12_1 <#if status==ALL>checked</#if>" name="${ALL}">全部</div>
      <div class="user_12_1 <#if status==OUT>checked</#if>" name="${OUT}">支出(-)</div>
      <div class="user_12_1 <#if status==IN>checked</#if>" name="${IN}">收入(+)</div>
   </div>
   ...
   <#--<#list detailList as detail>-->
       <#--<div class="user_13 <#if detail.detailType==IN>detailIn<#else >detailOut</#if>">-->
           ...
           <#--<span>现金余额：${detail.detailBalance}</span><span><#if detail.detailType==IN>+<#else >-</#if>${detail.detailMoney}</span>-->
       <#--</div>-->
   <#--</#list>-->
   <div id="data">
       <#--<div class="user_13">-->
