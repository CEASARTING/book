...
<body  class="body_1">
 <div class="header"><img src="/images/27.png" onclick="javascript :history.back(-1);"><span>订单详情</span></div>
  ...
  <div class="user_60"><span>取酒门店</span><span>${storeName!}</span></div>
  <div class="user_60"><span>取酒时间</span><span>${((ticket.order.orderFinishtime)?number_to_datetime)!}</span></div>
  <div class="user_61">
     <span>支付时间：</span><span>${((ticket.order.orderPayTime)?number_to_datetime)!}</span>
     <button>申请售后</button><button>再次购买</button>
  </div>
</body>
</html>
