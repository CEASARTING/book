...
<link rel="stylesheet" href="/css/C-main.css" type="text/css"/>
    ...
    <script type="text/javascript">
        function cancelOrder(){
            $.ajax({
                type: 'GET',
                url: '/c/order/cancel/${(ticket.order.orderId)!}',
                dataType: 'json',
        ...
        function pay(){
            if(${ticket.order.orderSource}==${buy}){
                window.location.href="/charge/index?orderId=${(ticket.order.orderId)!}&desc=${desc!}";
            }else if(${ticket.order.orderSource}==${buyStore}){
                window.location.href="/charge/index?orderId=${(ticket.order.orderId)!}&desc=${descStore!}";
            }else if(${ticket.order.orderSource}==${userBuyWine}){
                window.location.href="/charge/index?orderId=${(ticket.order.orderId)!}&desc=${descWine!}";
            }
        }
    </script>
...
<body  class="body_1">
 ...
 <div class="user_56" onclick=window.location.href="/c/store/suitStore?storesStr=${storesStr!}"><span>适用门店</span><span>${storeName!}${stores!}家</span><span>></span></div>
  <div class="user_57"><span>支付方式</span><span>支付方式</span></div>
  <div class="user_57"><span>选择红包</span><span>无</span></div>
  <div class="user_58">
      <span>购买酒量</span><span>${(ticket.order.orderTotal)!}ml</span>
     <span>原价</span><span>￥360.00</span>
     <span>优惠（含红包）</span><span>-￥30.00</span>
  </div>
  <div class="user_59"><span><d>实付款：</d><c>¥</c>320.00</span></div>
  <div class="user_62">
     <button onclick="cancelOrder()">取消订单</button><button onclick="pay()">立即支付</button>
  </div>
